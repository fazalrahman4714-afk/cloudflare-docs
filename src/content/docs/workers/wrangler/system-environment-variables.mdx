...
- `NODE_ENV` <Type text="string" /> <MetaInfo text="optional" />
  ...
  - Sets the value of `process.env.NODE_ENV` in your Worker code. Defaults to `"development"` for `wrangler dev` and `"production"` for `wrangler deploy`, `wrangler versions upload`, and `wrangler build`. Refer to [Bundling](/workers/wrangler/bundling/#node_env) for more information.
